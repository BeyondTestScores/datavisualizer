...
namespace :data do
  ...
  desc 'Bulk'
  task load_responses_bulk: :environment do
    school_names_whitelist = ['Vinson-Owen Elementary School']
    ...
    ['student_responses', 'teacher_responses'].each do |file|
      ...
      csv.each_with_index do |row, index|
        ...
        next unless school_names_whitelist.include?(school_name) 
    ...
    schools_whitelist = school_names_whitelist.map {|name| School.find_by_name(name) }
    recipients = schools_whitelist.flat_map {|school| school.recipients }
  ...
  task load_responses: :environment do
    ...
    schools_whitelist = [School.find_by_name('Vinson-Owen Elementary School')]
    schools_whitelist.each do |school|
